...
import type { WsConnectionStatus, Branch } from '@/lib/types'
...
export default function Topbar({ wsStatus, currentBranch, branches, isRunning, actions }: Props) {
  ...
  return (
    <header className="flex items-center gap-3 px-4 h-11 border-b border-[#1e1e1e] bg-[#000] flex-shrink-0">
      ...
      <div className="flex items-center gap-2 mr-2">
        ...
        <span className="text-[9px] font-mono text-[#333] border border-[#222] px-1.5 py-0.5 rounded">
          v0.1
        </span>
      </div>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-[11px] font-mono" style={{ color: branchColor }}>
          {currentBranch}
        </span>
      </div>
      ...
      <div className="flex items-center gap-1">
        {['FileSystem', 'Git', 'Shell', 'LLM', 'Memory'].map((tool, i) => {
      ...
      <div className="relative">
        ...
        >
          <span
            className={cn('w-1.5 h-1.5 rounded-full flex-shrink-0', s.animate && 'animate-pulse')}
        ...
        {showConnect && (
          <div className="absolute right-0 top-full mt-1 w-64 bg-[#0f0f0f] border border-[#1e1e1e] rounded-lg shadow-2xl z-50 p-3">
            ...
            <div className="flex gap-2">
              <button
                className="flex-1 text-[10px] font-mono py-1.5 rounded bg-[#0070f3] text-white hover:bg-[#0060d3] transition-colors"
